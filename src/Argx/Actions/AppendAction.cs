...
internal class AppendAction : ArgumentAction
{
    ...
    public override void Execute(Argument argument, IArgumentRepository repository, ReadOnlySpan<Token> tokens)
    {
        ...
        if (obj is IList src)
        {
            dest = CollectionUtils.CreateCollection(argument.Type, itemType, src.Count + len);
            CopyItems(src, dest);
            idx = src.Count;
        }
        else
        ...
        if (tokens.Length == 1)
        {
            ...
            {
                Append(dest, argument.ConstValue, isArray, idx);
                idx++;
            }
        ...
        for (int i = 1; i < tokens.Length; i++)
        {
            ...
            Append(dest, result.Value, isArray, idx);
            idx++;
        }

        repository.Set(argument.Dest, dest);
    }
