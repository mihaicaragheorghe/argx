...
By default, the parser parses and stores all the argument values, however this behavior can be changed using [actions](#actions). For example, in order to introduce a verbosity counted flag, we can decorate the above code with the following:
...
The help (`--help`, `-h`) argument is added by default and the help text is generated by the parser:
...
To parse collection arguments we can use [arity](#arity):
...
var parsedArgs = parser.Parse(args);
var files = parsedArgs.GetRequired<string[]>("files");
...
Many applications split their functionality into subcommands, for example, `git` has subcommands like `pull`, `push`, `status`, etc. Argx support subcommands via `CommandLineApplication`, which is similar to `WebApplication` in minimal APIs.
...
The `help` subcommand is added by default:
