...
By default, the parser parses and stores all the argument values, however this behavior can be changed using [actions](#actions). For example, if we need to introduce a verbosity counted flag, we can decorate the above code with the following:
...
Using [actions](#actions) and [arity](#arity) we can parse collection arguments:
